...
export default function AdminAccountsPage() {
    ...
    const openForm = (admin?: AdminNode) => {
        ...
        setValues(admin ? { userId: admin.user.id, employeeNumber: admin.employeeNumber, roleTitle: admin.position } : { userId: '', employeeNumber: '', roleTitle: '' });
        setOpen(true);
    };

    const save = async () => {
        if (!values.userId || !values.employeeNumber || !values.roleTitle) {
            setMessage('User ID, employee number, and role title are required.');
            return;
        }
        setSaving(true);
        setMessage(null);
        try {
            const api = getAdmins();
            const payload = {
                user_public_id: values.userId,
                employee_number: values.employeeNumber,
                role_title: values.roleTitle,
                first_name: editing?.user?.firstName ?? editing?.firstName ?? '',
                last_name: editing?.user?.lastName ?? editing?.lastName ?? '',
            ...
            <Container maxWidth="lg" sx={{ display: 'grid', gap: 3 }}>
                <Box sx={{ display: 'flex', justifyContent: 'space-between', gap: 2, alignItems: 'center' }}>
                    <Box>
                        ...
                        <Typography color="text.secondary">Manage administrator profiles.</Typography>
                    </Box>
                    <Button variant="contained" startIcon={<AddIcon />} onClick={() => openForm()}>Add administrator</Button>
                </Box>
                ...
                <Paper variant="outlined">
                    {loading ? (
                        ...
                        <List>
                            {data?.admins.edges.map(({ node }) => (
                                <ListItem
                                    key={node.id}
                                    secondaryAction={<Button startIcon={<EditIcon />} onClick={() => openForm(node)}>Edit</Button>}
                                >
                                    <ListItemText
                                        primary={`${node.user.firstName} ${node.user.lastName}`}
                                        secondary={`${node.employeeNumber} · ${node.position} · ${node.user.email}`}
                                    />
                                </ListItem>
                            ))}
                        </List>
                ...
                <Dialog open={open} onClose={() => !saving && setOpen(false)} fullWidth maxWidth="sm">
                    ...
                    <DialogContent sx={{ display: 'grid', gap: 2, pt: 2 }}>
                        ...
                        <TextField
                            ...
                            onChange={(event) => setValues({ ...values, employeeNumber: event.target.value })}
                            required
                        />
                        <TextField
                            label="Role title"
